...
public class Crawler {
	...
	"							<tr class=\"page\"><td>Page :	</td><td>" + docurl + "</td></tr>");
				...
				while (m.find())
				{
					...
					else
					{
						...
						else		//the path is from another domain.
						{
							...
							else
							{
								...
								if(ret[0]==0)
									BrokenLinks.add(new LinkStats(orglink,ret[1]));
							}
							LocalChecked.put(link,1);
							if(ret[0]==1)
								Working.add(new LinkStats(orglink,ret[1]));
							else
								Broken.add(new LinkStats(orglink,ret[1]));
							continue;
						}
					}
					...
					if(!RelLinks.containsKey(root+link))	//If the URL hasnt been checked then
					{
						...
						{
							Broken.add(new LinkStats(orglink,ret[1]));
							BrokenLinks.add(new LinkStats(root+link,ret[1]));
						}					
					}
					else
					{
				...
				//Now Add the Working,Broken links to HTML
				try{
	...
	"							<tr><td class=\"col1\"><b>HTTP<b></td><td class=\"col2\"><b>Link</b></td></tr>");
								for(loc=0;loc<Working.size();loc++)
								{
									fout.write("\r\n"+
	"							<tr><td class=\"col1\">"+ Working.get(loc).code + "</td><td class=\"col2\">" + Working.get(loc).url + "</td></tr>");
								}
					}
					else
					{
	...
	"							<tr class=\"broken\"><td colspan=2>Broken Links:-</td></tr>");
					...
					if(Broken.size()>0)
					{
						fout.write("\r\n" + 
	"							<tr><td class=\"col1\"><b>HTTP<b></td><td class=\"col2\"><b>Link</b></td></tr>");
								for(loc=0;loc<Broken.size();loc++)
								{
									fout.write("\r\n"+
	"							<tr><td class=\"col1\">"+ Broken.get(loc).code + "</td><td class=\"col2\">" + Broken.get(loc).url + "</td></tr>");
								}
					}
					else
					{
	...
	private void genReport(){
		...
		try{
			...
			fout.seek(1247);
			fout.writeBytes(String.valueOf(CrawlCount));
			fout.seek(1345);
			fout.writeBytes(String.valueOf(BrokenLinks.size()));
			fout.close();
...
"");
			...
			for(int loc=0;loc < BrokenLinks.size();loc++)
			{
				...
				fp.write("\r\n" +
"						<tr><td class=\"col1\">" + BrokenLinks.get(loc).code +"</td><td class=\"col2\">" + BrokenLinks.get(loc).url + "</td></tr>");
			}	
